...
public class DeleteCustomerControllerTests1 : IClassFixture<CustomerApiFactory>
{
    ...
    public DeleteCustomerControllerTests1(CustomerApiFactory apiFactory)
    {
    ...
    {
        // Act
        var deletedCustomer = await _client.DeleteAsync($"customers/{Guid.NewGuid()}");

        // Assert
        deletedCustomer.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }
}